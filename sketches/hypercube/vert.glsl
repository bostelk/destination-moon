...
vec4 hypercube_lines(float vertexId, float id)
{
...
void main()
{
  ...
  t4 *= rot4zx(phase * 0.1 * TAU);
  t4 *= rot4xy(rotation.x);
  t4 *= rot4yz(rotation.y);
  vec4 v4 = hypercube_lines(position.x, position.x);
  ...
  gl_Position = vec4(v2.xy * aspect * scale, 0, 1);
}
